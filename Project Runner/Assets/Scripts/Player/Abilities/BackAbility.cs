...
public class BackAbility : BodyPartAbility
{
    ...
    {
        // inputReader es inyectado por BodyPartManager via base.Initialize() — sin reflection
        if (inputReader == null)
        {
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (isDashing)
        {
            // Detectar colisi�n con CUALQUIER objeto s�lido, no solo "Obstacle"
            // Verificar si la colisi�n es frontal (en la direcci�n del dash)
            Vector3 collisionNormal = collision.contacts[0].normal;
            float dotProduct = Vector3.Dot(dashDirection, -collisionNormal);

            // Si la colisi�n es frontal (dot > 0.5), detener el dash
            if (dotProduct > 0.5f)
            {
                Debug.Log($"[LegsAbility] Dash interrumpido por colisi�n frontal con {collision.gameObject.name}");
